...
var selectV = false;
function cSelect(btnSelect, name){
	var choiceAreas = $("#"+btnSelect).children('.cArea');
	var selectValue = $("#"+btnSelect).children('.sValue');
	var zIndex = $("#"+btnSelect).attr("data-role");
	console.log(zIndex);
	$("#"+btnSelect).bind('tap click', function(){
		if (!selectV) {
			$("#"+btnSelect).css('z-index', zIndex);
			choiceAreas.show();
	...
	$(choiceAreas).children('span').bind('tap click', function(){
		$(choiceAreas).children('span').removeClass('on');
		$(this).addClass('on');
		textValue = $(this).text();
